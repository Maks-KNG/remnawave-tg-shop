...
from bot.utils.message_cleaner import send_clean  # <-- добавили
...
async def _initiate_yk_payment(
    ...
    *,
    ...
    if payment_response_yk and payment_response_yk.get("confirmation_url"):
        ...
        text_msg = get_text(key="payment_link_message", months=months)
        ...
        try:
            ...
            logging.warning(
                f"Edit message for payment link failed: {e_edit}. Sending new one via send_clean."
            )
            try:
                await send_clean(
...
async def select_subscription_period_callback_handler(callback: types.CallbackQuery, settings: Settings, i18n_data: dict, session: AsyncSession):
    ...
    try:
        ...
        logging.warning(
            f"Edit message for payment method selection failed: {e_edit}. Sending new one via send_clean."
        )
        try:
            await send_clean(
                callback.message.bot,
...
async def pay_yk_callback_handler(callback: types.CallbackQuery, settings: Settings, i18n_data: dict, yookassa_service: YooKassaService, session: AsyncSession):
    ...
    if autopay_enabled and saved_methods:
        keyboard = get_yk_autopay_choice_keyboard(
            months,
            price_rub,
            current_lang,
            i18n,
        ...
        try:
            ...
            logging.warning(f"Failed to show autopay choice: {e_edit}. Sending new message via send_clean.")
            try:
                await send_clean(
                    callback.message.bot,
                    callback.message.chat.id,
...
async def pay_yk_saved_list_handler(callback: types.CallbackQuery, settings: Settings, i18n_data: dict, yookassa_service: YooKassaService, session: AsyncSession):
    ...
    keyboard = get_yk_saved_cards_keyboard(
        cards,
        months,
        price_rub,
        current_lang,
    ...
    if success:
        ...
        if location:
            ...
            try:
                ...
                logging.warning(f"FreeKassa: failed to display payment link via edit_text ({e_edit}), sending new via send_clean.")
                try:
                    await send_clean(
                        callback.message.bot,
                        callback.message.chat.id,
